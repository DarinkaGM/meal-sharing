...
router.get("/", async (request, response) => {
  try {
    if (request.query.maxPrice) {
      ...
      const regularPrice = await knex("meal").where("Price", "<", maxPrice);
      response.json(regularPrice);
    } 
    
    if (request.query.availableReservations) {
      const getAvailableReservation = await knex("meal")
     ...
     if (request.query.title) {
      ...
      const getMatchingTitle = await knex("meal").where("title", "like", `%${matchingTitle}%`);
    ...
    if (request.query.createdAfter) {
      ...
      const newCreationDate = await knex("meal").where("created_date", ">", creationDate);
      response.json(newCreationDate);
    }
    
    if (request.query.limit) {
      const mealLimit = Number(request.query.limit);
      const limitingNumber = await knex("meal").select("*").limit(mealLimit);
